...
func TestUpdate(t *testing.T) {
	...
	updated, err := c.UpdateTestType(ctx, created.ID, &goclient.TestTypeRequest{Text: "bar"}, nil)
	require.NoError(t, err)
...
func TestUpdateNotExist(t *testing.T) {
	...
	updated, err := c.UpdateTestType(ctx, "doesnotexist", &goclient.TestTypeRequest{Text: "bar"}, nil)
	require.Error(t, err)
...
func TestUpdateIfMatchETagSuccess(t *testing.T) {
	...
	updated, err := c.UpdateTestType(ctx, created.ID, &goclient.TestTypeRequest{Text: "bar"}, &goclient.UpdateOpts{Prev: created})
	require.NoError(t, err)
	require.Equal(t, "bar", updated.Text)
